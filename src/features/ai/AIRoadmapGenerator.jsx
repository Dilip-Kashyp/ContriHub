...
import { generateRoadmap } from "@/helper";
...
export default function AIRoadmapGenerator({ isFullPage = false }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await generateRoadmap({ interest: finalInterest, skillLevel });
      ...
      <div className="bento-item" style={{ 
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <button 
            ...
            disabled={loading || (!interest && !customInterest.trim())} 
            style={{ 
              width: "100%",
              padding: "16px", 
              borderRadius: "14px", 
              ...
              fontSize: "1.05rem", 
              cursor: "pointer", 
              opacity: loading || (!interest && !customInterest.trim()) ? 0.5 : 1,
              transition: "all 0.3s ease",
              transform: loading ? "scale(0.98)" : "scale(1)"
            }}
          >
            {loading ? "Generating Your Path..." : "Build My Gibo Roadmap"}
          </button>
        </div>
      </div>
      <AIDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="Your Gibo Roadmap" response={response} loading={loading} error={error} />
    </>
  );
}
